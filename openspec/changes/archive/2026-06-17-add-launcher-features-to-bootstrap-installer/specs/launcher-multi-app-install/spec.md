# Capability: launcher-multi-app-install

## Purpose

Generalizes the existing single-app install and update flow so any
registered app can be installed, updated, repaired, or launched. The
existing `bootstrap.rs` and `update.rs` modules become parameterized
workers; the install-script protocol they speak (JSON manifest + per-stage
JSON result frames + line-buffered log streaming) is preserved unchanged.
