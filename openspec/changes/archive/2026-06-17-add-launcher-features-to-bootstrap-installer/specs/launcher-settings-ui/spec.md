# Capability: launcher-settings-ui

## Purpose

Provides the user-facing UI for the launcher in `--settings` mode: the
Home screen with app tiles, per-app drill-down (AppDetail), and
launcher-level configuration (Settings). All UI is implemented in React
+ nanostores, reusing the existing design system imported from
